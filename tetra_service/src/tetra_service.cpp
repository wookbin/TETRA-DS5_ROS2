...
public:
	TETRA_SERVICE() : Node("tetra_service")
	{
		...
		CALC_TF_timer_ = this->create_wall_timer(1000ms, std::bind(&TETRA_SERVICE::CALC_TF_Timer, this)); //1sec
	...
	*/
	...
	void CALC_TF_Timer()
	{
		if(m_bActive_map)
		{
			...
			{
				rclcpp::Time now = this->get_clock()->now();
				// Lookup the transform from the target frame to "odom"
				geometry_msgs::msg::TransformStamped transformStamped;
		...
		const std::shared_ptr<interfaces::srv::AddVirtualObstacles::Response> response)
	{
		...
		if (request->remove)  // Use "remove" instead of "delete"
		{
			...
			//printf("iID_array_size: %d \n", _pVirtual_polygon.iID_array_size);
			// Handle deletion
