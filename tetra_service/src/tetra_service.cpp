...
public:
	TETRA_SERVICE() : Node("tetra_service")
	{
		...
		virtual_obstacles_publisher = this->create_publisher<sensor_msgs::msg::PointCloud2>("virtual_obstacles", 10);
		virtual_obstacles_publisher2 = this->create_publisher<sensor_msgs::msg::PointCloud2>("virtual_obstacles2", 10);
	...
	*/
	...
	void ApriltagCallback(const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr msg) 
	{
		...
		if(size != 0) //detection maker
		{
			...
			//Transform Axis
			_pAR_tag_pose.m_dPositioning_Angle = _pAR_tag_pose.m_dApril_tag_yaw * (180.0/M_PI);
			_pAR_tag_pose.m_transform_pose_x = _pAR_tag_pose.m_dApril_tag_pose_z;
			_pAR_tag_pose.m_transform_pose_y = _pAR_tag_pose.m_dApril_tag_pose_x;
		...
		// //debuging//
		// printf("_pAR_tag_pose.m_dApril_tag_pose_x:  %.3f \n", _pAR_tag_pose.m_dApril_tag_pose_x);
		// printf("_pAR_tag_pose.m_dApril_tag_pose_y:  %.3f \n", _pAR_tag_pose.m_dApril_tag_pose_y);
		// printf("_pAR_tag_pose.m_dApril_tag_pose_z:  %.3f \n", _pAR_tag_pose.m_dApril_tag_pose_z);
		// printf("_pAR_tag_pose.m_dPositioning_Angle: %.3f \n", _pAR_tag_pose.m_dPositioning_Angle);
		// printf("----------------------------------- \n");
	}
	...
	bool ChargingStation_tracking(bool bOn, int marker_id)
	{
		...
		if(bOn)
		{
			...
			{	
				...
				if(m_fdistance > 0.5 /*&& m_fdistance < 1.5*/)
				{
	...
	{ 
		...
		{
			if((_pTF_pose.theta_deg * 2.0) <= (_pAR_tag_pose.m_target_theta * -1.0))
			{
				cmd.linear.x = 0.0; 
				cmd.angular.z = 0.0;
				cmd_vel_publisher->publish(cmd);
		...
		//printf(" _pAR_tag_pose.m_target_yaw : %.3f \n",_pAR_tag_pose.m_target_yaw );
		//printf("_pAR_tag_pose.m_target_theta: %.3f || _pTF_pose.theta_deg: %.3f \n", _pAR_tag_pose.m_target_theta, _pTF_pose.theta_deg);
	...
	bool ChargingStation_back_step()
	{
		...
		float m_fdistance = 0.0;
		m_fdistance = sqrt(_pAR_tag_pose.m_transform_pose_x * _pAR_tag_pose.m_transform_pose_x + _pAR_tag_pose.m_transform_pose_y * _pAR_tag_pose.m_transform_pose_y);
		//printf("Yaw_tracking2_distance: %.3f \n", m_fdistance);
		//printf("_pAR_tag_pose.m_target_theta : %.3f \n",_pAR_tag_pose.m_target_theta );

		//if(m_iBack_cnt < _pAR_tag_pose.m_target_theta * 30.0)
		if(m_iBack_cnt < 50)
		{
			if(_pFlag_Value.m_bFlag_Obstacle_Center)
			{
	...
	void TF_CALC_Timer()
	{
		...
		{
            RCLCPP_WARN(this->get_logger(), "Could not transform footprint: %s", ex.what());
        }
	...
	std::vector<geometry_msgs::msg::Point> generateDensePoints(const std::vector<geometry_msgs::msg::Point>& vertices, double resolution) 
	{
		...
		if (vertices.size() < 4) 
		{
			RCLCPP_ERROR(rclcpp::get_logger("virtual_obstacle_service"), "Insufficient vertices for a rectangle.");
			return dense_points;
		}
	...
	sensor_msgs::msg::PointCloud2 createPointCloud2(const std::vector<geometry_msgs::msg::Point>& dense_points) 
	{
		...
		cloud_msg2.header.frame_id = "odom"; //"map"; // Ensure this matches the global frame
	...
	void Add_Virtual_Obstacle_Command(
		const std::shared_ptr<interfaces::srv::AddVirtualObstacles::Request>  request, 
		const std::shared_ptr<interfaces::srv::AddVirtualObstacles::Response> response)
	{
		//printf("Add_Virtual_Obstacle_Command Service callback triggered. \n");

		if (request->remove)  // Use "remove" instead of "delete"
		{
			...
			for (const auto& id : request->ids) 
			{
				...
				{
					RCLCPP_INFO(this->get_logger(), "Global_costmap_Polygon with ID %d deleted.", id);
				} 
				else 
				{
					RCLCPP_WARN(this->get_logger(), "Global_costmap_Polygon with ID %d not found.", id);
				}

				if (polygon_local_costmap.erase(id) > 0) 
				{
					RCLCPP_INFO(this->get_logger(), "Local_costmap_Polygon with ID %d deleted.", id);
				} 
				else 
				{
					RCLCPP_WARN(this->get_logger(), "Local_costmap_Polygon with ID %d not found.", id);
				}
			}
			response->success = true;
			response->message = "Polygons deleted successfully.";
		} 
		else 
		{
			// Handle addition
			for (size_t i = 0; i < request->polygons.size(); ++i) 
			{
				...
				//printf("Polygon with ID %d added. \n", id);
			}
		...
		sensor_msgs::msg::PointCloud2 combined_cloud; //global
		for (const auto& [id, cloud] : polygon_global_costmap) 
		{
			mergePointClouds(combined_cloud, cloud);
		}
		virtual_obstacles_publisher->publish(combined_cloud);
		...
		virtual_obstacles_publisher2->publish(combined_cloud2);
...
private:
	...
	rclcpp::Service<interfaces::srv::AddVirtualObstacles>::SharedPtr virtual_obstacles_cmd_srv;
