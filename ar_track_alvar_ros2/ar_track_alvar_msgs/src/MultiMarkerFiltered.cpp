...
void MultiMarkerFiltered::PointCloudAverage(int marker_id, double edge_length,
                                            Pose& pose)
{
  if (marker_id == 0)
  {
  ...
  else
  {
    ...
    for (size_t j = 0; j < 4; ++j)
    {
      ...
      // The marker isn't used for pose calculation until we are quite sure
      // about it ???
                                    ...
                                    Camera* cam, Pose& pose, IplImage* image)
{
  ...
  for (MarkerIterator& i = begin.reset(); i != end; ++i)
  {
    ...
    if (marker_status[index] == 0)
    {
      ...
      cv::Mat cam_mat = cvMat(4, 4, CV_64F, cam_posed);
      cv::Mat mar_mat = cvMat(4, 4, CV_64F, mar_posed);
